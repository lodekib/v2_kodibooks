...
<body>
    ...
    <table class="table">
        ...
        <tbody>
            <tr>
                <td class="px-0">
                    @if($invoice->buyer->tenant)
                    <p class="seller-name">
                        <strong>{{ $invoice->buyer->tenant }}</strong>
                    </p>
                    @endif

                    @if($invoice->buyer->property)
                    <p class="seller-address">
                        Property Name : {{ $invoice->buyer->property }}
                    </p>
                    @endif

                    @if($invoice->buyer->org_address)
                    <p class="seller-code">
                        Organization Address : {{ $invoice->buyer->org_address }}
                    </p>
                    @endif

                    @if($invoice->buyer->tenant_email)
                    <p class="seller-vat">
                        Email Address : {{ $invoice->buyer->tenant_email }}
                    </p>
                    @endif

                    @if($invoice->buyer->due_date)
                    <p class="seller-phone">
                        Due Date : {{ $invoice->buyer->due_date }}
                    </p>
                    @endif
                </td>
